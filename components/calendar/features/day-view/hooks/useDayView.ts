...
export const useDayView = ({ date, onRefresh }: UseDayViewProps) => {
  ...
  const fetchClosedSlots = useCallback(async () => {
    try {
      ...
      // Sadece tarih bilgisini YYYY-MM-DD formatında gönderelim
      const formattedDate = format(date, "yyyy-MM-dd");

      console.log("İstek tarihi:", formattedDate);

      const response = await fetch(`/api/appointments/closed-slots?date=${formattedDate}`, {
        cache: 'no-store',
        headers: {
      ...
      if (response.ok) {
        ...
        // Backend'den gelen veriyi direkt olarak kullan
        setClosedSlots(data);
      } else {
